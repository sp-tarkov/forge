...
class AddonDependencyController extends Controller
{
     ...
     *
     * Resolves the complete dependency tree for one or more addon versions, returning all required mod dependencies.
    ...
    public function resolve(Request $request): JsonResponse
    {
        ...
        if (empty($addonsParam)) {
            return ApiResponse::error(
                "You must provide the 'addons' parameter.",
        ...
        // Look up addon version IDs from identifier:version pairs using service
        $queriedAddonVersionIds = $this->dependencyService->resolveAddonVersionIds($addonVersionPairs);

        if ($queriedAddonVersionIds->isEmpty()) {
            return ApiResponse::success([]);
        }

        // Build dependency trees for each queried addon version, including constraint information
        $allDependencies = collect();
        $constraintsByModId = collect();

        foreach ($queriedAddonVersionIds as $versionId) {
    ...
    private function deduplicateDependencies(Collection $dependencies, Collection $constraintsByModId): array
    {
        return $dependencies
            ...
            ->flatMap(function (Collection $modVersions, int $modId) use ($constraintsByModId) {
                ...
                // Get all constraints for this mod from all queried addon versions
    ...
    /**
     * Build the dependency tree for an addon version.
     *
     ...
     */
    private function buildDependencyTree(int $addonVersionId, Collection $processedVersionIds, Collection $constraintsByModId): ?array
    {
        // Use service to build the addon dependency tree
        $tree = $this->dependencyService->buildAddonDependencyTree($addonVersionId, $processedVersionIds, $constraintsByModId);
