...
describe('timestamp tracking', function (): void {
    it('does not update user mods_updated_viewed_at when page is viewed', function (): void {
        $user = User::factory()->create(['mods_updated_viewed_at' => null]);
        SptVersion::factory()->create(['version' => '3.11.4']);
    ...
    it('does not overwrite existing mods_updated_viewed_at timestamp on page view', function (): void {
        $oldTimestamp = now()->subWeek();
        $user = User::factory()->create(['mods_updated_viewed_at' => $oldTimestamp]);
        $storedTimestamp = $user->fresh()->mods_updated_viewed_at;
        SptVersion::factory()->create(['version' => '3.11.4']);

        $this->actingAs($user)->get(route('mods.recently-updated'));

        $user->refresh();
        expect($user->mods_updated_viewed_at->equalTo($storedTimestamp))->toBeTrue();
    });
...
describe('markAsRead action', function (): void {
    it('updates mods_updated_viewed_at when the mark-updated-as-read event is dispatched', function (): void {
        $user = User::factory()->create(['mods_updated_viewed_at' => null]);
        SptVersion::factory()->create(['version' => '3.11.4']);

        Livewire::actingAs($user)->test('pages::mod.recently-updated')
    ...
    it('clears the navigation badge cache when the mark-updated-as-read event is dispatched', function (): void {
        $user = User::factory()->create(['mods_updated_viewed_at' => null]);
        SptVersion::factory()->create(['version' => '3.11.4']);

        $cacheKey = sprintf('user:%s:nav-updated-mods-count', $user->id);
    ...
    it('leaves the currently visible list intact after the mark-updated-as-read event is dispatched', function (): void {
        $oldTimestamp = now()->subHours(2);
        $user = User::factory()->create(['mods_updated_viewed_at' => $oldTimestamp]);
        SptVersion::factory()->create(['version' => '3.11.4']);
