...
beforeEach(function (): void {
    ...
    // instead of marking the comment clean inline. The factory-seeded spam_status values these ribbon tests rely
    // on (PENDING, SPAM) would otherwise be overwritten by the inline disabled-path branch.
...
describe('Comment Spam Status Ribbons', function (): void {
    ...
    it('hides pending ribbon from comment author', function (): void {
        ...
        Livewire::actingAs($this->user)
            ->test('ribbon.comment', getCommentRibbonProps($comment, $this->user))
            ->assertDontSee('class="ribbon')
    ...
    it('hides pending ribbon from other users', function (): void {
        ...
        Livewire::actingAs($this->otherUser)
            ->test('ribbon.comment', getCommentRibbonProps($comment, $this->otherUser))
            ->assertDontSee('class="ribbon')
            ->assertDontSee('Pending');
    });
    ...
    it('shows pending ribbon to moderators who are not comment authors', function (): void {
        ...
        Livewire::actingAs($this->moderator)
            ->test('ribbon.comment', getCommentRibbonProps($comment, $this->moderator))
    ...
    it('hides pending ribbon from comment author even if they are admin', function (): void {
        ...
        Livewire::actingAs($this->admin)
            ->test('ribbon.comment', getCommentRibbonProps($comment, $this->admin))
            ->assertDontSee('class="ribbon')
            ->assertDontSee('Pending');
    });
