...
it('skips the broadcast refresh when notifications are unchanged', function (): void {
    ...
    DB::table('notifications')->insert([
        ...
        'data' => json_encode(['commenter_name' => 'Test User']),
        'read_at' => null,
        'created_at' => now(),
        'updated_at' => now(),
    ]);
...
it('listens on the user private broadcast channel', function (): void {
    ...
    $listeners = Livewire::test('navigation-notifications')
        ->instance()
        ->getListeners();

    expect($listeners)->toBe([
        sprintf('echo-private:user.%d,UserNotificationsChanged', $this->user->id) => 'refreshNotifications',
    ]);
});
