<div
                    ...
                    <flux:menu.group heading="Mod Actions">
                        ...
                        @if($this->permissions['delete'] ?? false)
                            <flux:modal.trigger name="mod-action-delete-{{ $this->modId }}">
                                <flux:menu.item icon:trailing="trash" variant="danger">Delete Mod</flux:menu.item>
                            </flux:modal.trigger>
                        @endif
                    </flux:menu.group>
                @endif
    ...
    {{-- Modals --}}
    <flux:modal name="mod-action-unfeature-{{ $this->modId }}" class="md:w-[500px] lg:w-[600px]">
        <div class="space-y-0">
            {{-- Header Section --}}
            <div class="border-b border-gray-200 dark:border-gray-700 pb-6 mb-6">
                <div class="flex items-center gap-3">
                    <flux:icon name="arrow-trending-down" class="w-8 h-8 text-amber-600" />
                    <div>
            ...
            <div class="space-y-4">
                ...
                <flux:text class="text-gray-700 dark:text-gray-300 text-sm">
                    This will remove the mod from the homepage featured section and remove the "featured" ribbon on its listing card.
                </flux:text>
            </div>

            {{-- Footer Actions --}}
            <div class="flex justify-end items-center pt-6 mt-6 border-t border-gray-200 dark:border-gray-700 gap-3">
                <flux:button x-on:click="$flux.modal('mod-action-unfeature-{{ $this->modId }}').close()" variant="outline" size="sm">
                    {{ __('Cancel') }}
                </flux:button>
                {{-- In the homepage featured section, the parent must handle the action so the listing can be updated --}}
                <flux:button x-on:click="$flux.modal('mod-action-unfeature-{{ $this->modId }}').close(); {{ ($homepageFeatured ? '$wire.$parent.unfeatureMod('.$this->modId.')' : '$wire.unfeature()') }}" variant="primary" size="sm" icon="arrow-trending-down" class="bg-amber-600 hover:bg-amber-700 text-white">
                    {{ __('Remove Featured') }}
                </flux:button>
            </div>
        </div>
    ...
    <flux:modal name="mod-action-feature-{{ $this->modId }}" class="md:w-[500px] lg:w-[600px]">
        <div class="space-y-0">
            {{-- Header Section --}}
            <div class="border-b border-gray-200 dark:border-gray-700 pb-6 mb-6">
                <div class="flex items-center gap-3">
                    <flux:icon name="sparkles" class="w-8 h-8 text-blue-600" />
                    <div>
                        <flux:heading size="xl" class="text-gray-900 dark:text-gray-100">
                            {{ __('Feature Mod') }}
                        </flux:heading>
            ...
            <div class="space-y-4">
                ...
                <flux:text class="text-gray-700 dark:text-gray-300 text-sm">
                    This will add the mod to the homepage featured section and add the "featured" ribbon on its listing card.
                </flux:text>
            </div>

            {{-- Footer Actions --}}
            <div class="flex justify-end items-center pt-6 mt-6 border-t border-gray-200 dark:border-gray-700 gap-3">
                <flux:button x-on:click="$flux.modal('mod-action-feature-{{ $this->modId }}').close()" variant="outline" size="sm">
                    {{ __('Cancel') }}
                </flux:button>
                <flux:button x-on:click="$flux.modal('mod-action-feature-{{ $this->modId }}').close(); $wire.feature()" variant="primary" size="sm" icon="sparkles" class="bg-blue-600 hover:bg-blue-700 text-white">
                    {{ __('Feature') }}
                </flux:button>
            </div>
        </div>
    ...
    <flux:modal name="mod-action-disable-{{ $this->modId }}" class="md:w-[500px] lg:w-[600px]">
        <div class="space-y-0">
            {{-- Header Section --}}
            <div class="border-b border-gray-200 dark:border-gray-700 pb-6 mb-6">
                <div class="flex items-center gap-3">
            ...
            {{-- Footer Actions --}}
            <div class="flex justify-end items-center pt-6 mt-6 border-t border-gray-200 dark:border-gray-700 gap-3">
                <flux:button x-on:click="$flux.modal('mod-action-disable-{{ $this->modId }}').close()" variant="outline" size="sm">
                    {{ __('Cancel') }}
                </flux:button>
                <flux:button x-on:click="$flux.modal('mod-action-disable-{{ $this->modId }}').close(); $wire.disable()" variant="primary" size="sm" icon="eye-slash" class="bg-amber-600 hover:bg-amber-700 text-white">
                    {{ __('Disable') }}
                </flux:button>
            </div>
        </div>
    ...
    <flux:modal name="mod-action-enable-{{ $this->modId }}" class="md:w-[500px] lg:w-[600px]">
        <div class="space-y-0">
            {{-- Header Section --}}
            <div class="border-b border-gray-200 dark:border-gray-700 pb-6 mb-6">
                <div class="flex items-center gap-3">
            ...
            {{-- Footer Actions --}}
            <div class="flex justify-end items-center pt-6 mt-6 border-t border-gray-200 dark:border-gray-700 gap-3">
                <flux:button x-on:click="$flux.modal('mod-action-enable-{{ $this->modId }}').close()" variant="outline" size="sm">
                    {{ __('Cancel') }}
                </flux:button>
                <flux:button x-on:click="$flux.modal('mod-action-enable-{{ $this->modId }}').close(); $wire.enable()" variant="primary" size="sm" icon="eye" class="bg-green-600 hover:bg-green-700 text-white">
                    {{ __('Enable') }}
                </flux:button>
            </div>
        </div>
    ...
    <flux:modal name="mod-action-delete-{{ $this->modId }}" class="md:w-[500px] lg:w-[600px]">
        <div class="space-y-0">
            {{-- Header Section --}}
            <div class="border-b border-gray-200 dark:border-gray-700 pb-6 mb-6">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <flux:text class="text-amber-800 dark:text-amber-300 text-sm mt-1">
                                The mod will be permanently deleted, along with all mod versions, mod comments, and any other data tied directly to the mod. If this isn't necessary, you should simply disable the mod instead.
                            </flux:text>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex justify-between items-center pt-6 mt-6 border-t border-gray-200 dark:border-gray-700">
                ...
                <div class="flex gap-3">
                    <flux:button x-on:click="$flux.modal('mod-action-delete-{{ $this->modId }}').close()" variant="outline" size="sm">
                        {{ __('Cancel') }}
                    </flux:button>
                    <flux:button x-on:click="$flux.modal('mod-action-delete-{{ $this->modId }}').close(); $wire.$parent.deleteMod({{ $this->modId }}, '{{ $routeName }}')" variant="primary" size="sm" icon="trash" class="bg-red-600 hover:bg-red-700 text-white">
                        {{ __('Delete') }}
                    </flux:button>
                </div>
            </div>
