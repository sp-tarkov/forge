<div
    ...
    <flux:modal name="mod-version-action-publish-{{ $this->versionId }}" class="md:w-[500px] lg:w-[600px]" x-data="{
            now() {
        ...
        <div class="space-y-0">
            ...
            <div class="space-y-4">
                <flux:text class="text-gray-700 dark:text-gray-300 text-sm">
                    Select the date and time version "{{ $this->versionNumber }}" of the "{{ $this->version->mod->name ?? 'Unknown' }}" mod will be published.
                </flux:text>
            ...
            {{-- Footer Actions --}}
            <div class="flex justify-end items-center pt-6 mt-6 border-t border-gray-200 dark:border-gray-700 gap-3">
                <flux:button x-on:click="$flux.modal('mod-version-action-publish-{{ $this->versionId }}').close()" variant="outline" size="sm">
                    {{ __('Cancel') }}
                </flux:button>
                <flux:button x-on:click="$flux.modal('mod-version-action-publish-{{ $this->versionId }}').close(); $wire.publish()" variant="primary" size="sm" icon="eye" class="bg-green-600 hover:bg-green-700 text-white">
                    {{ __('Publish') }}
                </flux:button>
            </div>
        </div>
    ...
    <flux:modal name="mod-version-action-unpublish-{{ $this->versionId }}" class="md:w-[500px] lg:w-[600px]">
        <div class="space-y-0">
            {{-- Header Section --}}
            <div class="border-b border-gray-200 dark:border-gray-700 pb-6 mb-6">
                <div class="flex items-center gap-3">
            ...
            <div class="space-y-4">
                <flux:text class="text-gray-700 dark:text-gray-300 text-sm">
                    Are you sure you want to unpublish version "{{ $this->versionNumber }}" of the "{{ $this->version->mod->name ?? 'Unknown' }}" mod?
                </flux:text>
                <flux:text class="text-gray-700 dark:text-gray-300 text-sm">
            ...
            {{-- Footer Actions --}}
            <div class="flex justify-end items-center pt-6 mt-6 border-t border-gray-200 dark:border-gray-700 gap-3">
                <flux:button x-on:click="$flux.modal('mod-version-action-unpublish-{{ $this->versionId }}').close()" variant="outline" size="sm">
                    {{ __('Cancel') }}
                </flux:button>
                <flux:button x-on:click="$flux.modal('mod-version-action-unpublish-{{ $this->versionId }}').close(); $wire.unpublish()" variant="primary" size="sm" icon="eye-slash" class="bg-amber-600 hover:bg-amber-700 text-white">
                    {{ __('Unpublish') }}
                </flux:button>
            </div>
        </div>
