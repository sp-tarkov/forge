<div x-data="{
                ...
                @if ($this->permissions['viewActions'] ?? false)
                    <flux:menu.group heading="Author Actions">
                        @if ($this->permissions['update'] ?? false)
                            <flux:menu.item
                                href="{{ route('addon.version.edit', [$this->addonId, $this->versionId]) }}"
                                icon="pencil"
                            >Edit Version</flux:menu.item>
                        @endif
                        @if ($this->versionPublished)
                            @if ($this->permissions['unpublish'] ?? false)
                                <flux:modal.trigger name="addon-version-action-unpublish-{{ $this->versionId }}">
                                    <flux:menu.item icon="eye-slash">Unpublish Version</flux:menu.item>
                                </flux:modal.trigger>
                            @endif
                        @else
                            @if ($this->permissions['publish'] ?? false)
                                <flux:modal.trigger name="addon-version-action-publish-{{ $this->versionId }}">
                                    <flux:menu.item icon="eye">Publish Version</flux:menu.item>
                                </flux:modal.trigger>
                            @endif
                        @endif
                        @if ($this->permissions['delete'] ?? false)
                            <flux:modal.trigger name="addon-version-action-delete-{{ $this->versionId }}">
                                <flux:menu.item
                                    icon="trash"
                                    variant="danger"
                    ...
                    <flux:menu.group heading="{{ auth()->user()->role->name }} Actions">
                        ...
                        @if ($this->versionDisabled)
                            @if ($this->permissions['enable'] ?? false)
                                <flux:modal.trigger name="addon-version-action-enable-{{ $this->versionId }}">
                                    <flux:menu.item icon="eye">Enable Version</flux:menu.item>
                                </flux:modal.trigger>
                            @endif
                        @else
                            @if ($this->permissions['disable'] ?? false)
                                <flux:modal.trigger name="addon-version-action-disable-{{ $this->versionId }}">
    ...
    {{-- Modals --}}
    <flux:modal
        name="addon-version-action-enable-{{ $this->versionId }}"
    ...
    >
        <div class="space-y-0">
            ...
            <div class="flex justify-end items-center pt-6 mt-6 border-t border-gray-200 dark:border-gray-700 gap-3">
                <flux:button
                    x-on:click="$flux.modal('addon-version-action-enable-{{ $this->versionId }}').close()"
                    variant="outline"
                    size="sm"
                >
                    {{ __('Cancel') }}
                ...
                <flux:button
                    x-on:click="$flux.modal('addon-version-action-enable-{{ $this->versionId }}').close(); $wire.enable()"
                    variant="primary"
                    size="sm"
                    icon="eye"
    ...
    <flux:modal
        name="addon-version-action-disable-{{ $this->versionId }}"
        class="md:w-[500px] lg:w-[600px]"
    >
        <div class="space-y-0">
            {{-- Header Section --}}
            ...
            <div class="flex justify-end items-center pt-6 mt-6 border-t border-gray-200 dark:border-gray-700 gap-3">
                <flux:button
                    x-on:click="$flux.modal('addon-version-action-disable-{{ $this->versionId }}').close()"
                    variant="outline"
                    size="sm"
                >
                    {{ __('Cancel') }}
                ...
                <flux:button
                    x-on:click="$flux.modal('addon-version-action-disable-{{ $this->versionId }}').close(); $wire.disable()"
                    variant="primary"
                    size="sm"
                    icon="eye-slash"
    ...
    <flux:modal
        name="addon-version-action-delete-{{ $this->versionId }}"
        class="md:w-[500px] lg:w-[600px]"
    >
        <div class="space-y-0">
            {{-- Header Section --}}
            ...
            <div class="flex justify-between items-center pt-6 mt-6 border-t border-gray-200 dark:border-gray-700">
                ...
                <div class="flex gap-3">
                    <flux:button
                        x-on:click="$flux.modal('addon-version-action-delete-{{ $this->versionId }}').close()"
                        variant="outline"
                        size="sm"
                    >
                        {{ __('Cancel') }}
                    ...
                    <flux:button
                        x-on:click="$flux.modal('addon-version-action-delete-{{ $this->versionId }}').close(); $wire.$parent.deleteAddonVersion({{ $this->versionId }})"
                        variant="primary"
                        size="sm"
                        icon="trash"
    ...
    <flux:modal
        name="addon-version-action-publish-{{ $this->versionId }}"
        class="md:w-[500px] lg:w-[600px]"
        x-data="{
            now() {
    ...
    >
        <div class="space-y-0">
            ...
            <div class="flex justify-end items-center pt-6 mt-6 border-t border-gray-200 dark:border-gray-700 gap-3">
                <flux:button
                    x-on:click="$flux.modal('addon-version-action-publish-{{ $this->versionId }}').close()"
                    variant="outline"
                    size="sm"
                >
                    {{ __('Cancel') }}
                ...
                <flux:button
                    x-on:click="$flux.modal('addon-version-action-publish-{{ $this->versionId }}').close(); $wire.publish()"
                    variant="primary"
                    size="sm"
                    icon="eye"
                    class="bg-green-600 hover:bg-green-700 text-white"
    ...
    <flux:modal
        name="addon-version-action-unpublish-{{ $this->versionId }}"
        class="md:w-[500px] lg:w-[600px]"
    >
        <div class="space-y-0">
            {{-- Header Section --}}
            ...
            <div class="flex justify-end items-center pt-6 mt-6 border-t border-gray-200 dark:border-gray-700 gap-3">
                <flux:button
                    x-on:click="$flux.modal('addon-version-action-unpublish-{{ $this->versionId }}').close()"
                    variant="outline"
                    size="sm"
                >
                    {{ __('Cancel') }}
                ...
                <flux:button
                    x-on:click="$flux.modal('addon-version-action-unpublish-{{ $this->versionId }}').close(); $wire.unpublish()"
                    variant="primary"
                    size="sm"
                    icon="eye-slash"
                    class="bg-amber-600 hover:bg-amber-700 text-white"
