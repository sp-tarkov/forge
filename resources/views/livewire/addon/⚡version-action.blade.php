...
new class extends Component {
    ...
    public function version(): AddonVersion
    {
        return AddonVersion::query()
            ->select(['id', 'version', 'disabled', 'published_at', 'addon_id'])
        ...
        <div class="space-y-0">
            ...
            <div class="flex justify-between items-center pt-6 mt-6 border-t border-gray-200 dark:border-gray-700">
                ...
                <div class="flex gap-3">
                    <flux:button
                        x-on:click="$wire.moderationReason = ''; $flux.modal('addon-version-action-delete-{{ $this->versionId }}').close()"
                        variant="outline"
                        size="sm"
                    >
                        {{ __('Cancel') }}
                    ...
                    <flux:button
                        x-on:click="$flux.modal('addon-version-action-delete-{{ $this->versionId }}').close(); $wire.$parent.deleteAddonVersion({{ $this->versionId }}, $wire.moderationReason)"
                        variant="primary"
                        size="sm"
                        icon="trash"
