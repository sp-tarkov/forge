<div class="space-y-6">
    ...
    @if($this->recentActivity->isEmpty())
        <div class="text-center py-12">
            ...
            <flux:text variant="muted" class="text-lg">No recent activity to display</flux:text>
            <flux:text variant="muted" size="sm" class="mt-1">Activity will appear here as the user interacts with the platform</flux:text>
        </div>
    @else
        <div class="relative">
            {{-- Timeline line --}}
            ...
            <div class="space-y-8">
                @foreach($this->recentActivity as $index => $event)
                    <div class="relative flex items-center space-x-8">
                        ...
                        <flux:badge variant="" color="{{ $this->getEventColor($event) }}" class="relative z-20 flex items-center justify-center w-12 h-12 rounded-full shadow-sm">
                            <flux:icon name="{{ $this->getEventIcon($event) }}" class="w-5 h-5 text-white" />
                        </flux:badge>
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center justify-between">
                                ...
                                <time class="text-sm text-gray-500 dark:text-gray-400 font-medium" datetime="{{ $event->created_at->toISOString() }}">
                            ...
                            {{-- Additional metadata --}}
                            <div class="mt-0.5 flex items-center flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500 dark:text-gray-400">
                                ...
                                @if($event->event_url)
                                    <a href="{{ $event->event_url }}" class="flex items-center space-x-1 text-white underline hover:text-gray-200 transition-colors">
