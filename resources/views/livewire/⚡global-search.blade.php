...
new class extends Component {
    ...
    /**
     * Rendering hook to execute search.
     */
    ...
    {
        /** @var array{hits: array<int, mixed>} $rawResults */
        $rawResults = Addon::search($query)
            ->options(['showRankingScore' => true])
            ->raw();

        /** @var Collection<int, mixed> $searchHits */
        $searchHits = collect($rawResults['hits'])->sortByDesc('_rankingScore')->values();
    ...
    <div class="w-full max-w-lg lg:max-w-md">
        ...
        >
            ...
            <div
                ...
                aria-live="polite"
                class="absolute z-20 top-11 w-full mx-auto max-w-2xl transform overflow-hidden rounded-md bg-white dark:bg-gray-900 shadow-2xl border border-gray-300 dark:border-gray-700 transition-opacity"
            >
                <div
                    x-cloak
                    x-show="count"
                >
                    ...
                    >
                        @foreach ($result as $type => $results)
                            @if ($results->count())
                                <h4
                                    x-on:click="is{{ Str::ucfirst($type) }}CatVisible = !is{{ Str::ucfirst($type) }}CatVisible; $wire.toggleTypeVisibility('{{ $type }}')"
                                ...
                                <div
                                    ...
                                    :class="{ 'max-h-screen': is{{ Str::ucfirst($type) }}CatVisible }"
                                >
                                    @foreach ($results as $hit)
                                        @component('components.global-search-result-' . Str::lower($type), [
                                            'result' => $hit,
                                            'linkClass' =>
                                                'group/global-search-link flex flex-row gap-3 py-1.5 px-4 text-gray-900 dark:text-gray-100 hover:bg-gray-200 dark:hover:bg-gray-800',
                                        ])
                                        @endcomponent
                                    @endforeach
                                </div>
                ...
                >
                    ...
                    <p class="mt-4 text-sm text-gray-900 dark:text-gray-200">
                        {{ __("We couldn't find any content with that query. Please try again.") }}</p>
                </div>
            </div>
        </search>
    </div>
