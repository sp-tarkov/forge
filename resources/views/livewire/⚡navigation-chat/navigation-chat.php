...
new class extends Component
{
    ...
    public function handleNewMessage(array $event): void
    {
        ...
        /** @var array{conversation_hash_id?: string}|null $message */
        $message = $event['message'] ?? null;
        if (request()->routeIs('chat') && is_array($message) && isset($message['conversation_hash_id'])) {
            $currentConversationHash = request()->route('conversationHash');
            if ($currentConversationHash === $message['conversation_hash_id']) {
                $user = Auth::user();
                $conversation = Conversation::query()->where('hash_id', $message['conversation_hash_id'])->first();
