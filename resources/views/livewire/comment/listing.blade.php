            ...
            @if ($comment->descendants->count() > 0)
                <div class="transition-all duration-600 mt-4" x-show="showReplies{{ $comment->id }}" x-collapse>
                    @foreach ($comment->descendants as $descendant)
                        <div
                            wire:key="comment-card-{{ $descendant->id }}"
                            class="p-6 mb-4 last:mb-0 bg-gray-50 dark:bg-gray-900 rounded-xl shadow-md dark:shadow-gray-950 drop-shadow-2xl filter-none transition-all duration-600"
                        >
