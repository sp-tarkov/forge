...
<div>
        ...
        <div class="space-y-6">
            ...
            <div id="filters-container" class="bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
                    ...
                    {{-- Role Filter --}}
                    <div>
                        <flux:label for="roleFilter" class="text-xs">Role</flux:label>
                        <flux:select wire:model.live="roleFilter" id="roleFilter" size="sm">
                    ...
                    {{-- Date Range Filters --}}
                    <div>
                        <flux:label for="joinedFrom" class="text-xs">Joined From</flux:label>
                        <flux:input type="date" wire:model.live="joinedFrom" id="joinedFrom" size="sm" />
                    </div>

                    <div>
                        <flux:label for="joinedTo" class="text-xs">Joined To</flux:label>
                        <flux:input type="date" wire:model.live="joinedTo" id="joinedTo" size="sm" />
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Users ({{ number_format($this->users->total()) }})</h3>
                </div>
                            ...
                            @forelse($this->users as $user)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                    ...
                                    <td class="px-2 sm:px-3 py-2 whitespace-nowrap">
                                        ...
                                        @else
                                            <flux:badge color="gray" size="sm">User</flux:badge>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-2 sm:px-3 py-2 whitespace-nowrap">
                                        ...
                                        @if($activeBan)
                                            <div class="flex items-center gap-2">
                                                ...
                                                <flux:tooltip>
                                                    <flux:icon.information-circle class="w-4 h-4 text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 cursor-help" />
                                                    <flux:tooltip.content>
                                                        <div class="text-sm">
                                                            <div class="font-medium">Ban Details</div>
                                                            <div class="mt-1">Reason: {{ $activeBan->comment ?: 'No reason provided' }}</div>
                                                            <div>Date: {{ $activeBan->created_at->format('M j, Y g:i A') }}</div>
                                                            <div>Expires: {{ $activeBan->expired_at ? $activeBan->expired_at->format('M j, Y g:i A') : 'Permanent' }}</div>
                                                        </div>
                                                    </flux:tooltip.content>
                                                </flux:tooltip>
                                            </div>
                                    ...
                                    <td class="px-2 sm:px-3 py-2 whitespace-nowrap text-center">
                                        ...
                                        @else
                                            <flux:icon.shield-exclamation class="w-4 h-4 text-red-600 dark:text-red-400 mx-auto" />
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-2 sm:px-3 py-2 whitespace-nowrap">
                                        <flux:dropdown align="end">
                                            ...
                                            <flux:menu>
                                                <flux:menu.item icon="eye" wire:navigate href="{{ $user->profile_url }}">
                                                    View Profile
                                                </flux:menu.item>
                                                <flux:menu.item icon="computer-desktop" wire:click="showUserIpAddresses({{ $user->id }})">
                                                    View IP Addresses
                                                </flux:menu.item>
                                                @if(!$user->isAdmin())
                                                    <flux:menu.separator />
                                                    @if($activeBan)
                                                        <flux:menu.item icon="check-circle" wire:click="showUnbanUser({{ $user->id }})">
                                                            Unban User
                                                        </flux:menu.item>
                                                    @else
                                                        <flux:menu.item icon="no-symbol" wire:click="showBanUser({{ $user->id }})">
                                                            Ban User
                                                        </flux:menu.item>
                                                    @endif
                                                @endif
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                        <flux:icon.users class="w-12 h-12 mx-auto mb-4 text-gray-300 dark:text-gray-600" />
                                        <p class="text-gray-500 dark:text-gray-400">No users found for the selected filters.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
    ...
    {{-- Ban User Modal --}}
    <flux:modal wire:model.self="showBanModal" class="md:w-[500px] lg:w-[600px]">
        <div class="space-y-0">
            {{-- Header Section --}}
            ...
            <div class="space-y-6">
                ...
                <div>
                    <flux:radio.group wire:model.live="banDuration" label="{{ __('Ban Duration') }}" class="text-left">
                        @foreach ($this->getDurationOptions() as $value => $label)
                            <flux:radio value="{{ $value }}" label="{{ $label }}" />
                        @endforeach
                    </flux:radio.group>
                    @error('banDuration')
                        <flux:text size="xs" variant="danger" class="mt-1">{{ $message }}</flux:text>
                    @enderror
                </div>
            ...
            <div class="flex justify-between items-center pt-6 mt-6 border-t border-gray-200 dark:border-gray-700">
                ...
                <div class="flex gap-3">
                    <flux:button wire:click="closeBanModal" variant="outline" size="sm">
                        {{ __('Cancel') }}
                    </flux:button>
                    <flux:button wire:click="banUser" variant="danger" size="sm" icon="shield-exclamation">
                        {{ __('Ban User') }}
                    </flux:button>
                </div>
            </div>
    ...
    {{-- Unban User Modal --}}
    <flux:modal wire:model.self="showUnbanModal" class="md:w-[400px]">
        <div class="space-y-0">
            {{-- Header Section --}}
            <div class="border-b border-gray-200 dark:border-gray-700 pb-6 mb-6">
                <div class="flex items-center gap-3">
            ...
            <div class="flex justify-end gap-3 pt-6 mt-6 border-t border-gray-200 dark:border-gray-700">
                <flux:button wire:click="closeUnbanModal" variant="outline" size="sm">
                    {{ __('Cancel') }}
                </flux:button>
                <flux:button wire:click="unbanUser" variant="primary" size="sm" icon="shield-check">
                    {{ __('Unban User') }}
                </flux:button>
            </div>
        </div>
    ...
    {{-- IP Addresses Modal --}}
    <flux:modal wire:model.self="showIpModal" variant="flyout">
        <div class="space-y-6">
            <div class="border-b border-gray-200 dark:border-gray-700 pb-6">
                <div class="flex items-center gap-3">
                    <flux:icon name="computer-desktop" class="w-8 h-8 text-blue-600" />
                    <div>
                        <flux:heading size="xl" class="text-gray-900 dark:text-gray-100">
                            IP Addresses
                        </flux:heading>
            ...
            <div class="space-y-4">
                ...
                <flux:callout variant="warning" icon="exclamation-triangle">
                    ...
                    <flux:callout.text>
                        <p class="mb-2">IP blocking should only be used in extreme cases, such as harassment or abuse situations. IP addresses may be shared among multiple users, masked by VPNs, or changed easily by users.</p>
                        <p><strong>Temporary blocks:</strong> IP bans are automatically set to expire after one month and should not be relied upon as a permanent solution.</p>
                    </flux:callout.text>
                </flux:callout>
                ...
                @forelse($userIpAddresses as $ipData)
                    <div class="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                        <div class="flex-1">
                            <div class="flex items-center gap-2">
                                <span class="font-mono text-sm text-gray-900 dark:text-gray-100">{{ $ipData->ip }}</span>
                                @if($ipData->is_banned)
                                    <flux:badge color="red" size="sm">Banned</flux:badge>
                                @endif
                            </div>
                            <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                <div>First seen: {{ \Carbon\Carbon::parse($ipData->first_seen)->format('M j, Y g:i A') }}</div>
                                <div>Last seen: {{ \Carbon\Carbon::parse($ipData->last_seen)->format('M j, Y g:i A') }}</div>
                                ...
                                <flux:tooltip>
                                    ...
                                    <flux:tooltip.content>
                                        <div class="text-sm">
                                            ...
                                            <div class="mt-1">This is your current IP address. Banning it would lock you out of the system.</div>
                                        </div>
                                    </flux:tooltip.content>
                                </flux:tooltip>
                            @else
            ...
            <div class="flex justify-end items-center pt-6 border-t border-gray-200 dark:border-gray-700">
                <flux:button wire:click="closeIpModal" variant="outline">
                    Close
                </flux:button>
            </div>
        </div>
