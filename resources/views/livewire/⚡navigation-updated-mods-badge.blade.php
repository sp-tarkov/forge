...
new class extends Component {
    ...
    /**
     * Load the count of mods updated since the user's last view, cached for 60 seconds.
     */
    private function loadUpdatedCount(): void
    {
        $user = Auth::user();

        if ($user === null || $user->mods_updated_viewed_at === null) {
            $this->updatedCount = 0;

            return;
        }

        $this->updatedCount = (int) Cache::remember(
            "user:{$user->id}:nav-updated-mods-count",
            60,
