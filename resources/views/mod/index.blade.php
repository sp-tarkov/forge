<x-app-layout>
    ...
    <div class="grid gap-6 grid-cols-1 lg:grid-cols-4 m-4">
        ...
        <div class="col-span-3">
            ...
            {{-- section filters --}}
            <div x-data="{selectedTab: 'new'}" class="hidden sm:block my-4">
                <nav class="isolate flex divide-x divide-gray-200 dark:divide-gray-800 rounded-xl shadow-md dark:shadow-gray-950 drop-shadow-2xl" aria-label="Tabs">
                    <button @click="selectedTab = 'new'" class="tab rounded-l-xl group relative min-w-0 flex-1 overflow-hidden py-4 px-4 text-center text-sm font-medium text-gray-900 dark:text-white bg-white dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-black dark:hover:text-white focus:z-10" aria-current="page">
                        <span>{{ __('New') }}</span>
                        <span aria-hidden="true" :class="selectedTab === 'new' ? 'bg-gray-500 absolute inset-x-0 bottom-0 h-0.5' : 'bottom-0 h-0.5'"></span>
                    </button>
                    <button @click="selectedTab = 'recently_updated'" class="tab group relative min-w-0 flex-1 overflow-hidden py-4 px-4 text-center text-sm font-medium text-gray-900 dark:text-white bg-white dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-black dark:hover:text-white focus:z-10">
                        <span>{{ __('Recently Updated') }}</span>
                        <span aria-hidden="true" :class="selectedTab === 'recently_updated' ? 'bg-gray-500 absolute inset-x-0 bottom-0 h-0.5' : 'bottom-0 h-0.5'"></span>
                    </button>
                    <button @click="selectedTab = 'most_downloaded'" class="tab group relative min-w-0 flex-1 overflow-hidden py-4 px-4 text-center text-sm font-medium text-gray-900 dark:text-white bg-white dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-black dark:hover:text-white focus:z-10">
                        <span>{{ __('Most Downloaded') }}</span>
                        <span aria-hidden="true" :class="selectedTab === 'most_downloaded' ? 'bg-gray-500 absolute inset-x-0 bottom-0 h-0.5' : 'bottom-0 h-0.5'"></span>
                    </button>
                    <button @click="selectedTab = 'top_rated'" class="tab rounded-r-xl group relative min-w-0 flex-1 overflow-hidden py-4 px-4 text-center text-sm font-medium text-gray-900 dark:text-white bg-white dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-black dark:hover:text-white focus:z-10">
                        <span>{{ __('Top Rated') }}</span>
                        <span aria-hidden="true" :class="selectedTab === 'top_rated' ? 'bg-gray-500 absolute inset-x-0 bottom-0 h-0.5' : 'bottom-0 h-0.5'"></span>
                    </button>
                </nav>
            </div>
