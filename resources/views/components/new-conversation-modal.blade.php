...
<flux:modal wire:model.live="{{ $showModal }}" class="md:w-[500px] lg:w-[600px]">
    <div class="space-y-0"
        ...
        <div class="space-y-4">
                    ...
                    <div class="space-y-2">
                        @foreach($searchResults as $user)
                            ...
                            >
                                ...
                                <flux:icon name="arrow-right" class="w-5 h-5 text-gray-400 dark:text-gray-600 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors" />
                            </button>
                        @endforeach
                    </div>
                </div>
            @elseif(!empty($searchUser))
                <div class="text-center py-8">
                    <flux:icon name="users" class="w-12 h-12 mx-auto text-gray-400 dark:text-gray-600 mb-3" />
            ...
            @else
                <div class="text-center py-8">
                    <flux:icon name="magnifying-glass" class="w-12 h-12 mx-auto text-gray-400 dark:text-gray-600 mb-3" />
                    <p class="text-gray-600 dark:text-gray-400">
        ...
        <div class="flex justify-end items-center pt-6 mt-6 border-t border-gray-200 dark:border-gray-700">
            <flux:button wire:click="closeNewConversationModal" variant="outline" size="sm">
                {{ __('Cancel') }}
            </flux:button>
        </div>
    </div>
