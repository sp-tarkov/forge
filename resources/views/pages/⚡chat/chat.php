...
new #[Layout('layouts::base')] class extends Component
{
    ...
    /**
     * The authenticated user's ID for JavaScript access.
     */
    ...
    public function boot(): void
    {
        ...
        // Only switch conversation if we don't already have one selected or if the hash doesn't match the current
        // selection
    ...
    public function handleIncomingMessage(array|bool $event): void
    {
        ...
        if (isset($event['message']) && is_array($event['message']) && isset($event['message']['user_id']) && is_int($event['message']['user_id'])) {
    ...
    public function sendMessage(): void
    {
        ...
        abort_if($user->cannot('sendMessage', $conversation), 403);
    ...
    public function archiveConversation(): void
    {
        ...
        if ($latestConversation && $latestConversation->hash_id) {
    ...
    public function handleForwardedMessage(array $event): void
    {
        ...
        if (! isset($event['message']) || ! is_array($event['message']) || ! isset($event['message']['conversation_hash_id'])) {
            return;
        }

        /** @var array{conversation_hash_id: string, user_id?: int} $message */
        ...
        if (! $this->selectedConversation instanceof Conversation) {
            ...
            $conversation = Conversation::query()->where('hash_id', $message['conversation_hash_id'])->forUser($user)->first();
        ...
        if ($this->selectedConversation->hash_id === $message['conversation_hash_id']) {
            // Clear typing indicator for the sender
            if (isset($message['user_id'])) {
                unset($this->typingUsers[$message['user_id']]);
            }
