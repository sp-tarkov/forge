        ...
        <div class="space-y-6">
                        ...
                        <div class="mt-4 divide-y divide-gray-700">
                            @forelse ($this->searchResults as $user)
                                <button
                                    ...
                                    class="flex items-center gap-3 w-full text-left py-3 hover:bg-gray-800 rounded-md px-2 -mx-2"
                                >
                                    <flux:avatar
                                        circle="circle"
            ...
            @else
                ...
                @if ($this->run?->isFailed())
                    <div
                        class="bg-gray-900 rounded-lg shadow-sm border border-gray-700 p-8 text-center">
                ...
                @elseif ($this->run?->isCompleted() && $this->result)
                    <div class="space-y-4">
                                ...
                                >
                                    @if (count($candidate->sharedIps) > 0)
                                        <div class="rounded-lg border border-gray-700 overflow-hidden">
                                            <div class="flex items-center gap-2 px-3 py-2 bg-gray-800/50">
                                                <flux:icon.globe-alt class="w-4 h-4 text-gray-500" />
                                                <span class="text-xs font-semibold uppercase tracking-wide text-gray-400">Shared IP addresses</span>
                                            </div>
                                            <div class="overflow-x-auto">
                                                <table class="w-full text-xs">
                                                    <thead>
                                                        <tr class="text-left text-gray-500 border-t border-gray-700">
                                                    ...
                                                    <tbody class="divide-y divide-gray-800 border-t border-gray-700">
                                                        @foreach ($candidate->sharedIps as $sharedIp)
                                                            <tr>
                                                                <td class="px-3 py-2 font-mono whitespace-nowrap text-gray-100 align-top">{{ $sharedIp->ip }}</td>
                                                                ...
                                                                <td class="px-3 py-2 font-mono whitespace-nowrap text-gray-400 align-top">{{ $sharedIp->firstSeen }}</td>
                                                                <td class="px-3 py-2 font-mono whitespace-nowrap text-gray-400 align-top">{{ $sharedIp->lastSeen }}</td>
                                                                <td class="px-3 py-2 align-top">
                                                                    @if (count($sharedIp->otherAccounts) > 0)
                                                                        <div class="flex flex-wrap gap-1 max-w-xs">
                                                                            @foreach ($sharedIp->otherAccounts as $otherAccount)
                                                                                <span class="rounded bg-gray-800 px-1.5 py-0.5 text-gray-300">{{ $otherAccount }}</span>
                                                                            @endforeach
                                                                        </div>
                                                                    @else
                                                                        <span class="text-gray-500 italic">Exclusive</span>
                                                                    @endif
                                                                </td>
                                                            </tr>
                                                        @endforeach
                                    ...
                                    @if ($candidate->sameDomain)
                                        <div class="rounded-lg border border-gray-700 px-3 py-2.5">
                                            <div class="flex items-center gap-2 mb-1.5">
                                                <flux:icon.envelope class="w-4 h-4 text-gray-500" />
                                                <span class="text-xs font-semibold uppercase tracking-wide text-gray-400">Email domain</span>
                                            </div>
                                            <div class="flex items-center gap-2">
                                                <span class="font-mono text-gray-100">{{ '@'.$candidate->domain }}</span>
                                    ...
                                    @if ($candidate->timeline)
                                        <div class="rounded-lg border border-gray-700 px-3 py-2.5">
                                            <div class="flex items-center gap-2 mb-1.5">
                                                ...
                                                <span class="text-xs font-semibold uppercase tracking-wide text-gray-400">Activity timeline</span>
                                            </div>
                                            <div>
                                                {{ $this->timelineLabel($candidate->timeline) }} on
                                                <span class="font-mono text-gray-100">{{ $candidate->timeline->ip }}</span>
                                            </div>
                                        </div>
                                    @endif
                                    ...
                                    @if (count($candidate->fingerprintOverlap) > 0)
                                        <div class="rounded-lg border border-gray-700 px-3 py-2.5">
                                            <div class="flex items-center gap-2 mb-1.5">
                                                <flux:icon.finger-print class="w-4 h-4 text-gray-500" />
                                                <span class="text-xs font-semibold uppercase tracking-wide text-gray-400">Device fingerprint</span>
                                            </div>
                                            <div class="flex flex-wrap gap-1.5">
                                                @foreach ($candidate->fingerprintOverlap as $fingerprint)
                                                    <span class="font-mono text-xs rounded bg-gray-800 px-2 py-1 text-gray-300">{{ str_replace('|', ' / ', $fingerprint) }}</span>
                                                @endforeach
                                            </div>
                                        </div>
                                    @endif
                        ...
                        @empty
                            <div
                                class="bg-gray-900 rounded-lg shadow-sm border border-gray-700 p-12 text-center">
                            ...
                            >
                                <div class="flex items-start justify-between gap-4">
                                    <div class="flex items-center gap-3 flex-1 min-w-0">
                                        <div class="w-8 h-8 rounded-full bg-gray-700 alt-skeleton-bar shrink-0">
                                        </div>
                                        <div class="flex-1 min-w-0 space-y-2">
                                            <div class="h-3.5 w-1/3 rounded bg-gray-700 alt-skeleton-bar">
                                            </div>
                                            <div class="h-2.5 w-1/2 rounded bg-gray-700 alt-skeleton-bar">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="h-5 w-12 rounded-full bg-gray-700 alt-skeleton-bar shrink-0">
                                    </div>
                                </div>
                                <div class="mt-3 flex flex-wrap gap-1.5">
                                    <div class="h-5 w-20 rounded-full bg-gray-700 alt-skeleton-bar"></div>
                                    <div class="h-5 w-24 rounded-full bg-gray-700 alt-skeleton-bar"></div>
                                    <div class="h-5 w-16 rounded-full bg-gray-700 alt-skeleton-bar"></div>
                                </div>
                            </div>
                        @endforeach
                    </div>
