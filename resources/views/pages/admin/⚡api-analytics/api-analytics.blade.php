            ...
            @else
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <flux:card class="space-y-1">
                        ...
                        <flux:heading size="xl">{{ number_format($this->summary['error_rate'], 2) }}%
                        </flux:heading>
                        <flux:text
                            size="sm"
                            class="text-gray-400"
                        >{{ number_format($this->summary['errors']) }} {{ __('errors') }}</flux:text>
                    </flux:card>
                    <flux:card class="space-y-1">
                        <flux:text size="sm">{{ __('Avg latency') }}</flux:text>
                        <flux:heading size="xl">
                            {{ $this->summary['avg_latency_ms'] !== null ? number_format($this->summary['avg_latency_ms']) . ' ms' : '-' }}
                        </flux:heading>
                    </flux:card>
                    <flux:card class="space-y-1">
                        <flux:text size="sm">{{ __('p95 latency') }}</flux:text>
                        <flux:heading size="xl">
                            {{ $this->summary['p95_latency_ms'] !== null ? number_format($this->summary['p95_latency_ms']) . ' ms' : '-' }}
                        </flux:heading>
                    </flux:card>
                </div>
                ...
                >
                    <div class="mb-4 flex items-center justify-between">
                        ...
                        <div class="text-sm text-gray-400">
                            <span x-show="! hovered">{{ __('Peak') }}: {{ number_format($this->peakRequests) }}
                                {{ __('req') }}</span>
                            <span
                                x-show="hovered"
                                x-text="hovered"
                    ...
                    <div class="flex gap-3">
                        {{-- Vertical scale --}}
                        <div class="flex h-40 w-14 shrink-0 flex-col justify-between text-right text-xs text-gray-500">
                            <span>{{ number_format($this->peakRequests) }}</span>
                            <span>{{ number_format((int) round($this->peakRequests / 2)) }}</span>
                            <span>0</span>
                        </div>
                        <div class="flex-1">
                            <div class="flex h-40 items-end gap-px border-b border-l border-gray-700">
                                @foreach ($this->timeSeries as $point)
                                    ...
                                    >
                                        <div
                                            class="w-full rounded-t bg-cyan-500/70 hover:bg-cyan-500"
                ...
                <div class="overflow-hidden rounded-lg border border-gray-700 bg-gray-900 shadow-sm">
                    <h3 class="p-6 pb-3 text-lg font-semibold text-gray-100">{{ __('Endpoints') }}</h3>
                            ...
                            <flux:table.rows>
                                @foreach ($this->endpoints as $endpoint)
                                    <flux:table.row :key="$endpoint['route_name']">
                                        <flux:table.cell class="font-mono text-xs">{{ $endpoint['route_name'] }}
                                        </flux:table.cell>
                                        <flux:table.cell align="end">{{ number_format($endpoint['requests']) }}
                                        </flux:table.cell>
                                        <flux:table.cell align="end">
                                            <flux:badge
                                                size="sm"
                                                :color="$endpoint['error_rate'] > 5 ? 'red' : ($endpoint['error_rate'] > 0 ?
                                                    'amber' : 'green')"
                                            >
                                                {{ number_format($endpoint['error_rate'], 1) }}%
                                            </flux:badge>
                                        </flux:table.cell>
                                        <flux:table.cell align="end">{{ number_format($endpoint['errors_4xx']) }} /
                                            {{ number_format($endpoint['errors_5xx']) }}</flux:table.cell>
                                        <flux:table.cell align="end">{{ number_format($endpoint['throttled']) }}
                                        </flux:table.cell>
                                        <flux:table.cell align="end">
                                            {{ $endpoint['avg_latency_ms'] !== null ? $endpoint['avg_latency_ms'] . ' ms' : '-' }}
                                        </flux:table.cell>
                                        <flux:table.cell align="end">
                                            {{ $endpoint['p95_latency_ms'] !== null ? $endpoint['p95_latency_ms'] . ' ms' : '-' }}
                                        </flux:table.cell>
                                    </flux:table.row>
                                @endforeach
                            </flux:table.rows>
                ...
                <div class="overflow-hidden rounded-lg border border-gray-700 bg-gray-900 shadow-sm">
                    <div class="flex items-baseline justify-between p-6 pb-3">
                        ...
                        <span class="text-sm text-gray-400">{{ number_format($this->unmatchedTotal) }}
                            {{ __('requests') }}</span>
                    </div>
                    ...
                    @if ($this->unmatchedRequests === [])
                        <p class="px-6 pb-6 text-sm text-gray-400">
                            {{ __('No unmatched paths recorded for this range.') }}</p>
                    @else
                        <div class="overflow-x-auto px-6 pb-4">
                            <flux:table>
                                <flux:table.columns>
                                ...
                                <flux:table.rows>
                                    @foreach ($this->unmatchedRequests as $unmatched)
                                        <flux:table.row
                                            :key="$unmatched['method'].
                                            ' '.$unmatched['status_code'].
                                            ' '.$unmatched['path']"
                                        >
                                            <flux:table.cell class="font-mono text-xs">{{ $unmatched['path'] }}
                                            </flux:table.cell>
                                            <flux:table.cell>
                                                <flux:badge
                                                    size="sm"
                                                    color="zinc"
                                                >{{ $unmatched['method'] }}</flux:badge>
                                            </flux:table.cell>
                                            <flux:table.cell align="end">
                                                <flux:badge
                                                    size="sm"
                                                    :color="$unmatched['status_code'] >= 500 ? 'red' : 'amber'"
                                                >
                                                    {{ $unmatched['status_code'] }}
                                                </flux:badge>
                                            </flux:table.cell>
                                            <flux:table.cell align="end">{{ number_format($unmatched['requests']) }}
                                            </flux:table.cell>
                                            <flux:table.cell
                                                align="end"
                                                class="whitespace-nowrap"
                                            >{{ $unmatched['last_seen']->diffForHumans() }}</flux:table.cell>
                                        </flux:table.row>
                                    @endforeach
                                </flux:table.rows>
                            </flux:table>
                ...
                {{-- Top clients --}}
                <div class="overflow-hidden rounded-lg border border-gray-700 bg-gray-900 shadow-sm">
                    <h3 class="p-6 pb-3 text-lg font-semibold text-gray-100">{{ __('Top clients') }}</h3>
                    @if ($this->topClients === [])
                            ...
                            <flux:table>
                                <flux:table.columns>
                                    ...
                                    <flux:table.column align="end">
                                        {{ $this->range === '24h' ? __('Active minutes') : __('Active days') }}
                                    </flux:table.column>
                                    <flux:table.column align="end">{{ __('Last seen') }}</flux:table.column>
                                </flux:table.columns>
                                <flux:table.rows>
                                    @foreach ($this->topClients as $client)
                                        <flux:table.row :key="$client['ip']">
                                            <flux:table.cell class="font-mono text-xs">{{ $client['ip'] }}
                                            </flux:table.cell>
                                            <flux:table.cell>
                                                @if ($client['country_name'] !== null)
                                                    <span
                                                        class="mr-1">{{ $client['flag'] }}</span>{{ $client['city_name'] !== null ? $client['city_name'] . ', ' : '' }}{{ $client['country_name'] }}
                                                @else
                                                    <span class="text-gray-500">{{ __('Unknown') }}</span>
                                                @endif
                                            </flux:table.cell>
                                            <flux:table.cell align="end">{{ number_format($client['requests']) }}
                                            </flux:table.cell>
                                            <flux:table.cell align="end">{{ number_format($client['share'], 1) }}%
                                            </flux:table.cell>
                                            <flux:table.cell align="end">
                                                {{ number_format($client['active_periods']) }}</flux:table.cell>
                                            <flux:table.cell
                                                align="end"
                                                class="whitespace-nowrap"
                                            >{{ $client['last_seen']->diffForHumans() }}</flux:table.cell>
                                        </flux:table.row>
                                    @endforeach
                                </flux:table.rows>
                            </flux:table>
